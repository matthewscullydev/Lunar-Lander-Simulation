...
//override update function for particle and change parameter to testLander




ThrustForce::ThrustForce(const ofVec3f& tthrust, float damp) {
	thrust = tthrust;
	damping = damp;
}
//TurbulenceForce::TurbulenceForce() {}
...
void ThrustForce::set(const ofVec3f& tthrust, float damp) {
	thrust = tthrust;
	damping = damp;
}
void ThrustForce::updateForce(Particle* particle) {
	...
	if (damping > 0.00) {
		...
		particle->forces.x += thrust.x *= damping;
		particle->forces.y += thrust.y *= damping;
		particle->forces.z += thrust.z *= damping;
	}
...
ImpulseForce::ImpulseForce(const ofVec3f& min, const ofVec3f& max, float damp) {
	tmin = min;
	tmax = max;
	damping = damp;
}
...
void ImpulseForce::set(const ofVec3f& min, const ofVec3f& max, float damp) {
	tmin = min;
	tmax = max;
	damping = damp;
}
void ImpulseForce::updateForce(Particle* particle) {
	...
	if (damping > 0.00) {
		damping -= 0.05;
		float num1 = ofRandom(tmin.x, tmax.x);
		float num2 = ofRandom(tmin.y, tmax.y);
		float num3 = ofRandom(tmin.z, tmax.z);
		particle->forces.x +=  num1 *= damping;
		particle->forces.y +=  num2 *= damping;
		particle->forces.z +=  num3 *= damping;
...
void ImpulseRadialForce::updateForce(Particle* particle) {
	...
	//
	if (height == 0) {
		ofVec3f dir = ofVec3f(ofRandom(-1, 1), this->height, ofRandom(-1, 1));
